...
namespace Shippo {
    public class APIResource {
        ...
        protected virtual HttpRequestMessage SetupRequest(string method, string url)
        {
            ...
            request.Headers.Add("Authorization", "ShippoToken " + accessToken);
            if (apiVersion != null)
            {
                request.Headers.Add("Shippo-API-Version", apiVersion);
            }
        ...
        public virtual string DoRequest(string endpoint, string method, string body)
        {
            ...
            var req = SetupRequest(method, endpoint);
            if (body != null)
            {
            ...
            try
            {
                var serializedContent = httpClient.SendAsync(req).Result;
                result = serializedContent.Content.ReadAsStringAsync().Result;
            }
            catch
            {
                return null;
