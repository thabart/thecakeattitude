...
import {UserService, SessionService, ProductsService, OpenIdService} from "../services";
        ...
        if (this.state.comments) {
            this.state.comments.forEach(function (comment) {
                ...
                comments.push((<div className="col-md-12">
                    {comment.subject === self.state.subject &&
                    <div className="close-comment"><i className="fa fa-times" onClick={() => {
                        self.displayRemoveComment(comment.id);
                    }}></i></div> }
        ...
        if (this.state.navigations && this.state.navigations.length > 1) {
            this.state.navigations.forEach(function (nav) {
                navigations.push((<li className="page-item"><a href="#" className="page-link" onClick={(e) => {
                    self.navigateComment(e, nav.name);
                }}>{nav.name}</a></li>));
            });
        }
            ...
            <section className="row white-section shop-section shop-section-padding">
                ...
                (<div className="col-md-12">
                    ...
                    <div className="form-group">
                        ...
                        <textarea className="form-control" placeholder="Your comment ..." name="comment"
                                  onChange={this.handleInputChange}/>
                    </div>
                    <div className="form-group">
                        {!this.state.isAddingComment ?
                            (<button className="btn btn-primary" onClick={this.addComment}>Add</button>) :
                            (<button className="btn btn-primary previous" disabled><i
                                className='fa fa-spinner fa-spin'></i> Processing update ...</button>)
                        }
                    </div>
                </div>)
                }
