...
class EditableTextArea extends Component {
  ...
  onClickField() { // Open the edit mode.
    this.setState({
      isEditMode: true
    });
  }
  ...
  render() { // Render the view.
    ...
    return <form onSubmit={(e) => { e.preventDefault(); this.validate(); }} className="row col-md-12">
      <div className="editable-input-container col-md-8">
        <Input type="textarea" state={feedbackName} className="form-control" value={this.state.oldValue} onChange={this.handleInputChange} minLength={this.props.minLength} maxLength={this.props.maxLength} />
...
export default translate('common', { wait: process && !process.release })(EditableTextArea);
