  ...
  render() {
    ...
    if (self.props.service.occurrence &&  self.props.service.occurrence !== null) {
      ...
      occurrence = (<table className="table table-striped">
        <tr><td>Start</td><td>{moment(self.props.service.occurrence.start_datetime).format('LLL')}</td></tr>
        <tr><td>End</td><td>{moment(self.props.service.occurrence.end_datetime).format('LLL')}</td></tr>
